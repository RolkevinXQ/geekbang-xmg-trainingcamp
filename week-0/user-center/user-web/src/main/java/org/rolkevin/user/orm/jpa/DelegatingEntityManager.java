package org.rolkevin.user.orm.jpa;
...
public class DelegatingEntityManager implements EntityManager {
    ...
    //通过spi方式知道，创建entityManager需要：
    //名称
    private String persistenceUnitName;

    //数据库配置properties,此处配置对应的properties文件路径
    private String propertiesLocation;

    private EntityManager entityManager;

    /**
     * jndi会根据所配置的内容注入初始值，所以需要setter方法
    ...
    public void init(){
        EntityManagerFactory entityManagerFactory =
                Persistence.createEntityManagerFactory(persistenceUnitName, loadProperties());
        this.entityManager = entityManagerFactory.createEntityManager();
    }

    private Properties loadProperties(){
        ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
        URL propertiesUrl =  classLoader.getResource(propertiesLocation);
